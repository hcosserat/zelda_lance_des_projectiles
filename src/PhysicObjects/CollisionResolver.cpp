...
bool CollisionResolver::buildC_C(Circle& A, Circle& B, Contact& c) {
    if (!A.collidesWith(B).collides) return false;
...
bool CollisionResolver::buildC_R(Circle& C, Rect& R, Contact& c) {
    const CollisionResult res = C.collidesWith(R);
...
bool CollisionResolver::buildContact(Actor& a, Actor& b, Contact& out) {
    const Shape sa = a.getShape();
    const Shape sb = b.getShape();

    if (sa == CircleShape && sb == CircleShape) return buildC_C(static_cast<Circle&>(a), static_cast<Circle&>(b), out);
    if (sa == CircleShape && sb == RectShape  ) return buildC_R(static_cast<Circle&>(a), static_cast<Rect&>(b),  out);

    if (sa == RectShape   && sb == CircleShape) {
        Contact tmp;
        if (!buildC_R(static_cast<Circle&>(b), static_cast<Rect&>(a), tmp)) return false;
...
void CollisionResolver::resolve(std::vector<Actor*>& actors) {
    // Lister les contacts
    ...
    for (size_t i = 0; i < actors.size(); ++i) {
        for (size_t j = i + 1; j < actors.size(); ++j) {
            Contact c;
            if (buildContact(*actors[i], *actors[j], c)) {
