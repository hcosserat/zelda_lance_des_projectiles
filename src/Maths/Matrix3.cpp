...
// Opérations de base
Matrix3 Matrix3::operator+(const Matrix3& M) const { return Matrix3(Vector(a.x+M.a.x, a.y+M.a.y, a.z+M.a.z),
                                                                    Vector(b.x+M.b.x, b.y+M.b.y, b.z+M.b.z),
                                                                    Vector(c.x+M.c.x, c.y+M.c.y, c.z+M.c.z)); }
Matrix3 Matrix3::operator-(const Matrix3& M) const { return Matrix3(Vector(a.x-M.a.x, a.y-M.a.y, a.z-M.a.z),
                                                                    Vector(b.x-M.b.x, b.y-M.b.y, b.z-M.b.z),
                                                                    Vector(c.x-M.c.x, c.y-M.c.y, c.z-M.c.z)); }
Matrix3 Matrix3::operator*(float s) const          { return Matrix3(Vector(a.x*s, a.y*s, a.z*s),
                                                                    Vector(b.x*s, b.y*s, b.z*s),
                                                                    Vector(c.x*s, c.y*s, c.z*s)); }
Matrix3& Matrix3::operator*=(float s) { a.x*=s; a.y*=s; a.z*=s; b.x*=s; b.y*=s; b.z*=s; c.x*=s; c.y*=s; c.z*=s; return *this; }
...
Matrix3 Matrix3::inverse(float eps) const {
    ...
    return Matrix3(Vector(c0.x*invDet, c0.y*invDet, c0.z*invDet),
                   Vector(c1.x*invDet, c1.y*invDet, c1.z*invDet),
                   Vector(c2.x*invDet, c2.y*invDet, c2.z*invDet));
}
