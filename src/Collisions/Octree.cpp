...
// Distance squared
// ------------------------------------------------------------
static float distanceSquared(const Vector & a, const Vector & b) {
	float dx = a.x - b.x;
	float dy = a.y - b.y;
	float dz = a.z - b.z;
...
bool Octree::contains(const RigidBody * body) const {
	...
	return body->center.x + r >= center.x - halfSize && body->center.x - r <= center.x + halfSize && body->center.y + r >= center.y - halfSize && body->center.y - r <= center.y + halfSize && body->center.z + r >= center.z - halfSize && body->center.z - r <= center.z + halfSize;
}
...
bool Octree::intersects(const Vector & point, float radius) const {
	...
	return distanceSquared(point, { x, y, z }) <= radius * radius;
}
...
void Octree::query(const Vector & point, float radius,
	...
	for (auto * e : elements) {
		...
		if (distanceSquared(point, e->center) <= r * r)
			result.push_back(e);
	}
